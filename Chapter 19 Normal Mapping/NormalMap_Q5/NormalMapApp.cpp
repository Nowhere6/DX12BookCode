...
void NormalMapApp::Draw(const GameTimer& gt)
{
  ...
  mCommandList->SetPipelineState(mPSOs["displace"].Get());
  DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Displacement]);
...
void NormalMapApp::BuildShadersAndInputLayout()
{
  ...
  mShaders["HS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "HS", "hs_5_1");
  mShaders["DS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "DS", "ds_5_1");
...
void NormalMapApp::BuildPSOs()
{
  ...
  ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&opaquePsoDesc, IID_PPV_ARGS(&mPSOs["displace"])));
  opaquePsoDesc.HS = D3D12_SHADER_BYTECODE{};
  opaquePsoDesc.DS = D3D12_SHADER_BYTECODE{};
  opaquePsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
  //
  // PSO for sky.
  //
...
void NormalMapApp::BuildRenderItems()
{
  ...
  mRitemLayer[(int)RenderLayer::Displacement].push_back(gridRitem.get());
